...
//Задание 7
/*void SendMessageToServers(wstring& serverName, wstring& message) {
    wstring mailslotName = L"\\\\";
    mailslotName.append(serverName);
    mailslotName.append(L"\\mailslot\\Box");
    HANDLE cH = CreateFile(mailslotName.c_str(), GENERIC_WRITE, 0, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
    ...
    DWORD bytesSended;
    if (!WriteFile(cH, message.c_str(), (message.size() + 1) * sizeof(wchar_t), &bytesSended, NULL)) {
...
int main() {
    ...
    */
    ...
    //wstring nameServer = L"sf";
    //wstring message = L"Hello from client";
