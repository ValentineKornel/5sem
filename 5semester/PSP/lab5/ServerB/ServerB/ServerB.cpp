...
int main()
{
    ...
    try {
        ...
        if (gethostname(hostName, sizeof(hostName)) == SOCKET_ERROR)
            throw SetErrorMsgText("hostname: ", WSAGetLastError());

        cout << "Server's hostname: " << hostName << endl;
        ...
        for (;;) {
            ...
            if((hostClnt = gethostbyaddr((char*)& clnt.sin_addr, sizeof(clnt.sin_addr), AF_INET)) == NULL)
                throw SetErrorMsgText("hostname: ", WSAHOST_NOT_FOUND);

            cout << hostClnt->h_name << " " << inet_ntoa(clnt.sin_addr) << ":" << htons(clnt.sin_port) << endl;

            if (PutAnswerToClient(name, (struct sockaddr*)&clnt, &lclnt)) {
                cout << "answered the client" << endl;
            }
        }
