...
HANDLE hAcceptServer, hConsolePipe, hGarbageCleaner, hDispatchServer, hResponseServer, hScheduleBox;
...
DWORD WINAPI ConsolePipeSocket(LPVOID pPrm);
...
struct Schedule {
    tm rand_start_tm, rand_end_tm;
    tm time_start_tm, time_end_tm;
    tm echo_start_tm, echo_end_tm;
} schedule;
...
bool compare_hour_minute(const std::tm& tm1, const std::tm& tm2) {
            ...
            for (auto client = contacts.begin(); client != contacts.end(); client++)
            {
                if (client->type == client->ACCEPT)
                {
                    ...
                    else
                    {
                        ...
                        tm* current_time = std::localtime(&now);
                        if (strcmp(client->msg, "Echo") == 0) {
                            if (compare_hour_minute(*current_time, schedule.echo_start_tm) || compare_hour_minute(schedule.echo_end_tm, *current_time)) {
                                if ((send(client->s, "Sorry! This service currently unavailable", strlen("Sorry! This service currently unavailable") + 1, NULL)) == SOCKET_ERROR)
                                    throw  SetErrorMsgText("send:", WSAGetLastError());
                                client->sthread = client->ABORT;
                                client->type = client->EMPTY;
                                if (closesocket(client->s) == SOCKET_ERROR)
                        ...
                        if (strcmp(client->msg, "Time") == 0) {
                            if (compare_hour_minute(*current_time, schedule.time_start_tm) || compare_hour_minute(schedule.time_end_tm, *current_time)) {
                                if ((send(client->s, "Sorry! This service currently unavailable", strlen("Sorry! This service currently unavailable") + 1, NULL)) == SOCKET_ERROR)
                                    throw  SetErrorMsgText("send:", WSAGetLastError());
                                client->sthread = client->ABORT;
                                client->type = client->EMPTY;
                        ...
                        if (strcmp(client->msg, "Rand") == 0) {
                            if (compare_hour_minute(*current_time, schedule.rand_start_tm) || compare_hour_minute(schedule.rand_end_tm, *current_time)) {
                                if ((send(client->s, "Sorry! This service currently unavailable", strlen("Sorry! This service currently unavailable") + 1, NULL)) == SOCKET_ERROR)
                                    throw  SetErrorMsgText("send:", WSAGetLastError());
                                client->sthread = client->ABORT;
                                client->type = client->EMPTY;
...
void parse_schedule(char* message) {
    // Переменные для хранения значений
    int rand_start_hour, rand_start_min, rand_end_hour, rand_end_min;
    int time_start_hour, time_start_min, time_end_hour, time_end_min;
    int echo_start_hour, echo_start_min, echo_end_hour, echo_end_min;
    ...
    while (std::getline(stream, segment, ';')) {
        ...
        std::string label = segment.substr(0, colon_pos);
        std::string times = segment.substr(colon_pos + 1);

        // Разделяем по "-"
        size_t dash_pos = times.find('-');
        if (dash_pos == std::string::npos) continue;

        std::string start_time = times.substr(0, dash_pos);
        std::string end_time = times.substr(dash_pos + 1);

        // Разделяем start_time и end_time по ":"
        int start_hour, start_min, end_hour, end_min;
        sscanf(start_time.c_str(), "%d:%d", &start_hour, &start_min);
        sscanf(end_time.c_str(), "%d:%d", &end_hour, &end_min);

        // Присваиваем значения в зависимости от label
        if (label == "RAND") {
            rand_start_hour = start_hour;
            rand_start_min = start_min;
            rand_end_hour = end_hour;
            rand_end_min = end_min;
        }
        else if (label == "TIME") {
            time_start_hour = start_hour;
            time_start_min = start_min;
            time_end_hour = end_hour;
            time_end_min = end_min;
        }
        else if (label == "ECHO") {
            echo_start_hour = start_hour;
            echo_start_min = start_min;
            echo_end_hour = end_hour;
            echo_end_min = end_min;
        }
    }

    tm rand_start, rand_end, time_start, time_end, echo_start, echo_end;
    rand_start.tm_hour = rand_start_hour;
    rand_start.tm_min = rand_start_min;
    rand_end.tm_hour = rand_end_hour;
    rand_end.tm_min = rand_end_min;

    time_start.tm_hour = time_start_hour;
    time_start.tm_min = time_start_min;
    time_end.tm_hour = time_end_hour;
    time_end.tm_min = time_end_min;

    echo_start.tm_hour = echo_start_hour;
    echo_start.tm_min = echo_start_min;
    echo_end.tm_hour = echo_end_hour;
    echo_end.tm_min = echo_end_min;

    if (compare_hour_minute(rand_end, rand_start) || compare_hour_minute(time_end, time_start) || compare_hour_minute(echo_end, echo_start)) {
        cout << "Error setting new Schedule. Incorrect format given\n";
        return;
    }

    EnterCriticalSection(&scSchedule);
    schedule.rand_start_tm = rand_start;
    schedule.rand_end_tm = rand_end;

    schedule.time_start_tm = time_start;
    schedule.time_end_tm = time_end;

    schedule.echo_start_tm = echo_start;
    schedule.echo_end_tm = echo_end;

    cout << "New Schedule: \n";
    std::cout << "RAND: " << schedule.rand_start_tm.tm_hour << ":" << schedule.rand_start_tm.tm_min << " - "
        << schedule.rand_end_tm.tm_hour << ":" << schedule.rand_end_tm.tm_min << std::endl;
    std::cout << "TIME: " << schedule.time_start_tm.tm_hour << ":" << schedule.time_start_tm.tm_min << " - "
        << schedule.time_end_tm.tm_hour << ":" << schedule.time_end_tm.tm_min << std::endl;
    std::cout << "ECHO: " << schedule.echo_start_tm.tm_hour << ":" << schedule.echo_start_tm.tm_min << " - "
        << schedule.echo_end_tm.tm_hour << ":" << schedule.echo_end_tm.tm_min << std::endl;
    LeaveCriticalSection(&scSchedule);
}

DWORD WINAPI ScheduleBox(LPVOID pPrm) {
    TalkersCommand& param = *((TalkersCommand*)pPrm);
    ...
    while (param != EXIT) {
        if (!ReadFile(hMailBox, rbuf, sizeof(rbuf) - 1, &bytesRead, NULL)) {
            DWORD error = GetLastError();
            if (error == ERROR_SEM_TIMEOUT) {
                //cout << "Timeout expired while reading from Mailslot.\n";
            }
            else {
...
int main(int argc, char* argv[])
{
    ...
    else if (argc == 4)
    {
        ...
        //set mail box parameter about how often chek mail box
        readMailBoxDelay = atoi(argv[4]);
        serverPort = atoi(argv[1]);
        strcpy(dllName, argv[2]);
        strcpy(namedPipeName, argv[3]);
    }
    ...
    hMailBox = CreateMailslotW(TEXT("\\\\.\\mailslot\\Box"), 1000, 0, NULL);
    ...
    char start_schedule[] = "RAND:0:00-23:59; TIME:0:00-23:59; ECHO:0:00-23:59";
    parse_schedule(start_schedule);
    ...
    hResponseServer = CreateThread(NULL, NULL, ResponseServer,
        (LPVOID)&cmd, NULL, NULL);
    hScheduleBox = CreateThread(NULL, NULL, ScheduleBox,
        (LPVOID)&cmd, NULL, NULL);
    ...
    SetThreadPriority(hScheduleBox, THREAD_PRIORITY_IDLE);
    ...
    TerminateThread(hScheduleBox, 0);
    puts("shutdown ScheduleBox");
    CloseHandle(hScheduleBox);
