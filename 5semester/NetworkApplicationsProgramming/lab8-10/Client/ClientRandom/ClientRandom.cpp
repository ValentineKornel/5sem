...
)
{
    ...
    all.sin_addr.s_addr = inet_addr("192.168.43.158");
...
int main()
{
    ...
    try {
        ...
        printf_s("\n ---------- Доступные команды ---------- \n");
        printf_s("1 - поиск с помощью широковещательного запроса\n");
        printf_s("2 - подключение по заданному ip адресу\n");
        printf_s("3 - подключение по DNS\n");
        int code = 0;
        printf_s("Команда: ");
        scanf_s("%d", &code);
        getchar();
        if (code > 0 && code < 4) {
            if (code == 1) {
                ...
                struct hostent* he;
                if ((he = gethostbyname("DESKTOP-0M3BPJP")) == NULL)
